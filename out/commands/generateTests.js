...
- Keep tests realistic and runnable
      `, editorContext);
        const response = await (0, openaiClient_1.askOpenAI)(context, prompt, prompts_1.SYSTEM_PROMPTS.generateTests);
        (0, output_1.showAssistantResponse)("Generated Tests", response);
    }
    catch (error) {
        vscode.window.showErrorMessage("Failed to generate tests.");
        (0, output_1.showError)("Failed to generate tests.", error);
    }
}
